"""
LiveKit + Gemini + Ditto Conversational Avatar Agent (SYNCED VERSION)
...
- Ditto for avatar video generation (always running at 60 FPS)

IMPROVEMENTS OVER BASE VERSION:
1. AVSynchronizer: Proper audio-video timestamp coordination (<50ms sync)
2. Proactive state transitions: 300ms silence timeout (no waiting for SDK message)
3. Buffer flushing: Processes remaining 0-6479 samples at speech end
4. Immediate silent audio: Event-based trigger eliminates 0-200ms frame gaps
...
Usage:
    ...
    python livekit_gemini_agent_synced.py dev
"""
...
class ConversationStateManager:
    ...
    State Machine (simplified):
        ...
        SPEAKING → IDLE (when 300ms silence detected OR Gemini finishes)
    ...
    IMPROVEMENTS:
    - Proactive timeout-based state transition (no waiting for SDK message)
    - Tracks last audio timestamp for silence detection
    """
    ...
    def __init__(self):
        ...
        # Proactive state transition tracking
        self.last_audio_timestamp = 0.0
        self.silence_timeout = 0.3  # 300ms silence = transition to IDLE
        ...
        # For 60 FPS: need to match frame generation rate
    ...
    def update_audio_timestamp(self):
        """Update last audio timestamp (for timeout-based transition)."""
        self.last_audio_timestamp = time.time()

    def get_silence_duration(self) -> float:
        """Get duration of silence since last audio."""
...
class AVSynchronizer:
    """
    Audio-Video timestamp synchronizer for LiveKit.

    Coordinates audio and video timestamps to ensure proper sync in browser.
    Without this, audio and video have different RTP timestamp origins,
    causing 605ms desync in our case.

    Strategy:
    - Establish base timestamp when first media is generated
    - Calculate relative timestamps for all subsequent frames
    - Audio: timestamp = (chunks_sent * chunk_duration) + base_timestamp
    - Video: timestamp = (ditto_timestamp) + base_timestamp
    """

    def __init__(self):
        self.base_timestamp_us = None  # Base timestamp in microseconds
        self.audio_chunks_sent = 0
        self.audio_chunk_duration_us = 20000  # 20ms = 20000 microseconds (LiveKit standard)

    def initialize(self):
        """Initialize base timestamp (call when first media is ready)."""
        if self.base_timestamp_us is None:
            self.base_timestamp_us = int(time.time() * 1_000_000)  # Current time in microseconds
            logger.info(f"🕐 AVSynchronizer initialized with base timestamp: {self.base_timestamp_us}μs")

    def get_audio_timestamp_us(self) -> int:
        """Get synchronized timestamp for next audio frame (in microseconds)."""
        if self.base_timestamp_us is None:
            self.initialize()

        # Calculate audio timestamp based on chunks sent
        timestamp_us = self.base_timestamp_us + (self.audio_chunks_sent * self.audio_chunk_duration_us)
        self.audio_chunks_sent += 1
        return timestamp_us

    def get_video_timestamp_us(self, ditto_timestamp: float) -> int:
        """
        Get synchronized timestamp for video frame (in microseconds).

        Args:
            ditto_timestamp: Ditto's internal timestamp (in seconds, video timeline)

        Returns:
            Synchronized timestamp in microseconds
        """
        if self.base_timestamp_us is None:
            self.initialize()

        # Convert Ditto timestamp (seconds) to microseconds and add to base
        ditto_timestamp_us = int(ditto_timestamp * 1_000_000)
        return self.base_timestamp_us + ditto_timestamp_us

    def reset(self):
        """Reset synchronizer (e.g., when conversation restarts)."""
        self.base_timestamp_us = None
        self.audio_chunks_sent = 0
        logger.info("🔄 AVSynchronizer reset")
...
class GeminiDittoAgent:
    ...
    SYNCED VERSION IMPROVEMENTS:
    - Audio-video timestamp coordination (AVSynchronizer)
    - Proactive state transitions with 300ms timeout
    - Buffer flushing at speech end
    - Immediate silent audio trigger (no gaps)
    """
    ...
    def __init__(
        ...
        **ditto_kwargs
        ...
        # Audio-video synchronization (NEW)
        ...
        # Silent audio generation task and trigger (NEW)
        self.silent_audio_task: Optional[asyncio.Task] = None
        self.silent_audio_trigger = asyncio.Event()  # Event-based trigger for immediate feed

        # State transition monitor task (NEW)
    ...
    async def initialize(self):
        ...
        logger.info("✅ AVSynchronizer ready for audio-video coordination")
    ...
    def _on_frame_generated(self, frame_rgb: np.ndarray, frame_idx: int, timestamp: float):
        ...
        if self._frames_generated == 1:
            ...
            # Initialize AVSynchronizer when first frame arrives
            self.av_sync.initialize()
        ...
        if self.video_source:
            try:
                ...
                # Note: Let LiveKit auto-assign timestamps for natural sync
    ...
    async def _capture_frame_async(self, video_frame: rtc.VideoFrame, stage_start: float):
        ...
        Args:
            video_frame: The video frame to capture
            stage_start: Start time for profiling
        """
        try:
            capture_start = time.perf_counter()
            # Let LiveKit auto-assign timestamp for natural sync
            ...
            if response.server_content.model_turn and response.server_content.model_turn.parts:
                ...
                for part in response.server_content.model_turn.parts:
                    if part.inline_data and part.inline_data.mime_type.startswith("audio/"):
                        ...
                        # Update timestamp for proactive transition monitoring (NEW)
                ...
                # NOTE: We no longer transition to IDLE here (reactive approach removed)
                # Instead, the state monitor task handles proactive timeout-based transition
    ...
    async def monitor_state_transitions(self):
        ...
        NEW: Proactive SPEAKING→IDLE transition when 300ms silence detected.
        This eliminates waiting for Gemini SDK's "no more audio" message,
        preventing audio feeding gaps.
        """
        logger.info("🔍 Starting state transition monitor (300ms silence timeout)")

        while True:
            try:
                # Check every 50ms for responsive transitions
                await asyncio.sleep(0.05)

                # Only monitor when SPEAKING
                if self.state_manager.is_speaking():
                    silence_duration = self.state_manager.get_silence_duration()

                    # If 300ms silence detected, transition to IDLE
                    if silence_duration >= self.state_manager.silence_timeout:
                        logger.info(f"⏱️  Detected {silence_duration*1000:.1f}ms silence - transitioning to IDLE")

                        # Flush partial audio buffer (NEW)
                        await self._flush_audio_buffer()

                        # Transition to IDLE
                        await self.state_manager.transition_to(ConversationState.IDLE)

                        # Trigger immediate silent audio feed to prevent gap (NEW)
    ...
    async def _flush_audio_buffer(self):
        ...
        NEW: Processes remaining 0-6479 samples by padding to 6480 and feeding to Ditto.
        This prevents audio loss at the end of utterances.
        """
        if len(self.gemini_audio_buffer) > 0:
            logger.info(f"🚰 Flushing {len(self.gemini_audio_buffer)} remaining samples from audio buffer")
            ...
            try:
                ...
                logger.info(f"✅ Flushed audio buffer (padded {padding_size} samples)")
            except Exception as e:
        ...
        if self.audio_source:
            ...
            for i in range(0, len(audio_48khz_int16), samples_per_chunk):
                ...
                if len(chunk_48k) == samples_per_chunk:
                    ...
                    # Note: AudioSource.capture_frame() does not support timestamp_us parameter
                    # LiveKit automatically assigns timestamps to audio frames
    ...
    async def run_silent_audio_generator(self):
        """
        Continuously feed silent audio to Ditto when not in SPEAKING state.
        This keeps the avatar visible with idle/listening animation at 60 FPS.

        IMPROVED: Event-based trigger eliminates 0-200ms gaps on state transitions.
        Uses asyncio.Event to send silent audio immediately when transitioning to IDLE.
        ...
        - Feeding continuously maintains steady frame rate
        """
        logger.info("🔇 Starting silent audio generator for idle/listening states (60 FPS)")
        ...
        logger.info(f"   Feed rate: Continuous (keeps GPU warm)")
        logger.info(f"   Event-based trigger: Eliminates 0-200ms gaps on transitions")

        while True:
            try:
                state = self.state_manager.get_state()

                # Only send silent audio when IDLE (not speaking)
                if state == ConversationState.IDLE:
                    stage_start = time.perf_counter()
                    silent_chunk, chunksize = self.state_manager.get_silent_audio()

                    # Feed silent audio to Ditto to maintain frames
                    ditto_start = time.perf_counter()
                    logger.debug(f"🔇 StreamSDK: Feeding silent audio chunk ({len(silent_chunk)} samples, chunksize={chunksize})")
                    await asyncio.to_thread(
                        self.sdk.run_chunk,
                        silent_chunk,
                        chunksize
                    ...
                    ditto_time = (time.perf_counter() - ditto_start) * 1000
                    self._profile_stage('ditto_chunk', ditto_time, f"Silent audio chunk ({len(silent_chunk)} samples)")
                    logger.debug(f"✅ StreamSDK: Silent audio chunk processed ({ditto_time:.2f}ms)")

                    total_time = (time.perf_counter() - stage_start) * 1000
                    self._profile_stage('silent_audio_gen', total_time, f"Total silent audio generation")

                # Wait for either: timeout (200ms) OR immediate trigger event (NEW)
                # This allows immediate silent audio feed on state transition
                try:
...
async def entrypoint(ctx: JobContext):
    """
    LiveKit + Gemini agent entrypoint (SYNCED VERSION).
    ...
    logger.info("✨ SYNCED VERSION - Audio-Video Synchronized + Proactive Transitions")
    ...
    # Start background tasks (NEW: Added state monitor)
    ...
    state_monitor_task = asyncio.create_task(agent.monitor_state_transitions())

    logger.info("✅ Background tasks started (Gemini + silent audio + state monitor)")
    ...
    logger.info("   - Avatar always visible with smooth animation at 60 FPS")
    logger.info("   - Audio-video synchronized within <50ms")
    logger.info("   - Proactive state transitions (300ms timeout)")
    logger.info("   - No audio gaps or frame freezes")
    ...
    logger.info(f"   - Target FPS: 60 (smooth for all content)")
    logger.info(f"   - Frame interval: 16.67ms")
    logger.info(f"   - Audio chunk: 6480 samples @ 16kHz")
    logger.info(f"   - Chunksize: (3, 5, 2) → 5 frames per chunk")
    logger.info(f"   - AVSynchronizer: Coordinating audio-video timestamps")

    # Keep alive and wait for all tasks
    try:
        await asyncio.gather(gemini_task, silent_audio_task, state_monitor_task)
    except asyncio.CancelledError:
        pass
    finally:
        # Cancel tasks
...
if __name__ == "__main__":
    """
    Start the LiveKit + Gemini agent (SYNCED VERSION).
